...
| `control-frame.ts` | Pitch/roll in −1..1, quality in 0..1, button state and one-consumer edges. | Pure read-only device contract with no host, firmware or transport facts. `m5-controller.ts` maps pitch/roll onto Control's `forwardTilt`/`rightTilt` sample. Neutral axes preserve level forward flight. |
