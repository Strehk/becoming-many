...
- Start owns four world-space flight goals and one bounded crossing observation.
  Its optional particle effect forms rings/arrows from a fixed 1,400-point draw
  and applies the local wake. It reads the shared viewpoint; background Air
  Particles, locomotion, audio and Show transport keep their existing owners.
  Show may hold presentation for an active instruction; only spatial passage
  completes a goal. The current recipe has no approved tutorial audio assets.
