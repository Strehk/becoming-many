...
import { protectControl } from "../../../src/m5/runtime/control-safety";
...
describe("control safety", () => {
  ...
  it.each([{ pitch: 0.85 }, { pitch: -0.85 }, { roll: 0.85 }, { roll: -0.85 }])(
    "rejects the inclusive resume limit on either axis: %p",
    (pose) => {
      expect(protectControl(createNeutralControl(), liveFrame(pose))).toEqual(
        createNeutralControl(),
      );
    },
  );
  ...
  it.each([
    ...
    { roll: 0.849 },
    { roll: -0.849 },
  ])("allows a resumed pose just inside the limit: %p", (pose) => {
    const frame = liveFrame(pose);
    expect(protectControl(createNeutralControl(), frame)).toEqual(frame);
  });
});
