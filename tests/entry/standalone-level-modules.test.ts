/**
 * Purpose: Verify loading of concrete modules owned by the standalone-level entry.
...
import { loadStandaloneLevelModules } from "../../src/entry/standalone-level-modules";
...
test("loads no Zone Visualizer without its terrain presentation", async () => {
  for (const preset of [whiteWorldLevel, designTestLevel]) {
    expect(await loadStandaloneLevelModules(preset)).toEqual({
...
test("loads Zone Visualizer for the diagnostic Test preset", async () => {
  const modules = await loadStandaloneLevelModules(testLevel);
