...
- Close, replace, or rewrite findings that upstream work has already resolved
  or invalidated.
- Never merge this snapshot wholesale. Extract only reviewed changes into a
  fresh branch based on current upstream.
...
| #37 | `replace-station-broker-with-broadcast-channel.md` | Replace the station broker with `BroadcastChannel` |
...
1. Start from a clean worktree based on the latest upstream `main`.
2. Reproduce or prove one issue against that exact revision.
3. Confirm scope, ownership, priority, and acceptance criteria.
4. Create one focused branch for that issue.
5. Implement the smallest complete solution and remove superseded code.
6. Run focused checks and the repository-wide test, type, lint, build, and
   Fallow gates.
7. Verify the real browser, control, XR, or PC VR path required by the issue.
