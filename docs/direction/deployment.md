...
- **PICO enterprise headset** connected to the station PC through a wired USB
...
The application runs and renders on the Windows station PC. A later standalone
PICO edition belongs in a separate reduced fork; it is not a second target
carried by this repository.

One property of this path simplifies browser integration: the page is served
from **`http://localhost`** on the station PC — a secure context for WebXR with
no mixed-content blocking for plain `http://` polling of LAN devices. TLS on
embedded devices, relay processes, and BLE are not needed for this topology.
...
3. **Exact streaming matrix.** Confirm the headset edition, PICO OS, PICO
   ...
   port before the headset spike. Streaming behavior binds to that complete
   tested matrix.
