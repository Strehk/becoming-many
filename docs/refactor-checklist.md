...
Use this branch-local checklist for one issue and one pull request at a time.
GitHub is the authority for live issue state. An unchecked item is a candidate,
not proof that the finding still exists.

Check an issue only after its implementation and evidence satisfy the definition
of done and its pull request is ready to merge. If current `main` already
resolved it and no deliverable diff remains, check it only after recording the
required evidence and closing the issue directly.

## Branch-Local Planning

`david/refactor-foundation` is the long-lived planning and working branch.
This checklist and every file under `docs/todo/` stay on that branch and must
never be included in a pull request or merged into `main`.

Implement one issue at a time on the foundation branch, but keep its deliverable
code, tests, and affected as-built documentation in commits separate from todo
and checklist updates. For delivery, create a fresh issue branch from current
`main` and cherry-pick only those deliverable commits. The pull-request diff must
contain neither `docs/todo/**` nor `docs/refactor-checklist.md`.

The first issue session is #37. Current `main` already records that the
one-window Conductor deleted the WebSocket broker and cross-window wire. After
the absence and runtime checks pass, close #37 directly with the evidence; it
needs no delivery branch or pull request because it has no remaining code diff.
...
- [ ] Refresh the foundation branch from current `origin/main` and record that
      base revision before implementation.
- [ ] If current baseline health is unknown, run
...
- [ ] Review the complete diff and staged scope. Keep deliverable code, tests,
      and as-built documentation separate from todo and checklist updates.
- [ ] Create an English imperative deliverable commit, for example
      `security: stop storing Wi-Fi passwords`.
- [ ] Create `david/issue-<number>-<short-slug>` from current `main` and
      cherry-pick only the deliverable commit or commits.
- [ ] Verify branch status, commit contents, ancestry, and that
      `git diff --name-only origin/main...HEAD -- docs/todo docs/refactor-checklist.md`
      produces no paths.
- [ ] With user approval, push the delivery branch and open a focused pull
...
- [ ] Resolve review and CI findings on the same branch and rerun affected
      checks.
- [ ] Check the issue in this branch-local queue only when all required evidence
      passes and the pull request is ready to merge. If upstream already resolved
      the issue and no deliverable diff remains, record the evidence and close it
      directly instead.
- [ ] After merge, refresh the foundation branch from `main` before the next
      Codex session. Do not continue work on the merged delivery branch.
...
- [x] [#37 Verify Removal of the Superseded Station Transport](https://github.com/Strehk/becoming-many/issues/37)
      — resolved in code by `9982d18`; absence, repository, benchmark, and
      production-browser checks passed on 2026-09-02, so no delivery PR is
      required.
