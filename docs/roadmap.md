...
| Implemented | Previous #80 → #73 → #20 block committed at `0a746e7`; continuation authorized after its concrete report. Next-session plan committed at `c38a867`. New block #27 → #26 → #32: 2/3 implemented; #27 is closed, #26 removes synchronous queue fallback with bounded retry and compact visibility storage. #26 remains open for tuning and physical acceptance; #32 is next. |
...
| Current work | #26 keeps rejected work in existing fixed slots, hides invalid assignments and publishes only complete current writes. The visibility buffer uses one byte instead of four. Production +7 lines, existing tests +114; no new files, owners or dependencies. Final Scent median/p95/p99 0.1/2.4/2.5 ms in all three headed runs, unchanged counters/queue. Replay stalls remain; no physical 90 Hz claim. See #26 and [performance](performance.md). |
...
authorized; the next block is at 2/3. Early physical commissioning is not
