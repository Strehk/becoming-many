...
| Bank appearance | [#81](https://github.com/Strehk/becoming-many/issues/81): proposed 1 m lateral distance from the existing analytic channel edge, canopy overhang allowed. In 42 inspected near-bank candidates this preserves the current rendering and restores three missing Scent/Connections projections; 2.5 m removes those three bushes. [Decision views](evidence/README.md) are committed; this remains an unapproved proposal, not a global placement guarantee. | #81 visible placement change and subsequent #41 |
