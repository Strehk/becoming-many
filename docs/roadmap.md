...
| Current work | #35 is pushed at `e811596`: Run diagnostic forwarding removed (production −16). #13 is pushed at `34f3e02`: legacy Grass and its exclusive paths retired (production −672, tests −418); #40 closed by that removal. #72 is pushed at `37b801b`: conservative bounds (+20 production), with six fewer draws than disabled culling in both compared views; no timing-speedup claim. #71 adds shared World Surface influences, removes hard visual selection and duplicate smoothing (+46 production, +207 tests). #28 validates both surviving wind consumers with existing tests and a real peak-gust browser run; no code/test additions. #81 is closed and pushed at `d3a4e3b`: one approved clearance rule (production −30, tests +16), with 42-candidate projection agreement. #41 shares fixed-slot lifecycle, queue, publication and pool construction between Rocks and Vegetation (production −309, tests −2), following the user’s explicit request for shared infrastructure. Concrete material/transform policy remains local; no general module framework. The user clarified that the next integration target is the #50 flight tutorial. Before it, the approved #85 follow-up makes all nine levels self-contained literal data and deletes ten authored files (+761 configuration lines, explicitly accepted); #73 removes redundant Composition setup/forwarding (−60 runtime lines). All nine effective configurations match exactly, targeted tests/build/lint pass. Tutorial content/start decisions and the original #73 clock uncertainty remain open. Integrated #13/#71/#72 visual and Windows-PCVR acceptance remain open. |
