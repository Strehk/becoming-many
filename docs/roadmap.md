...
| Current work | Size correction verified; focused local commits authorized. Continue with bounded #83 pointer-lock diagnosis. #22 closed not planned after read-only reconciliation; no dependency change or implementation-count increment. |
...
| Open acceptance | Original #75 strict-start cause unassigned despite a distinct #79 fix; #83 pointer-lock ground-view attempt failed; actual listening/visual review, explicit #78 reference approval and M0 acceptance remain open. |
